elasticnet_auc_analysis_grouped <- function(data_combined,
                                            outcome_cols,
                                            predictor_cols,
  ...
  for (outcome in outcome_cols) {
    pb$tick()
    cat("\nCurrently processing module:", outcome, "\n")
    ...
    if (all(is.null(model_list))) {
      cat("  No valid models for module:", outcome, "\n")
    ...
    if (length(selected) == 0) {
      cat("  No predictors selected for module:", outcome, "\n")
      skipped_outcomes <- add_row(skipped_outcomes,
                                  outcome = outcome,
