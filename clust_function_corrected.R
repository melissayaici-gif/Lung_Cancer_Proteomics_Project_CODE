elasticnet_auc_analysis_clusters <- function(data_combined,
                                             outcome_cols,
                                             predictor_cols,
  ...
  # Filter outcomes that have at least min_cases
  outcome_cols <- outcome_cols[sapply(outcome_cols, function(outcome) {
    sum(data_combined[[outcome]] == 1, na.rm = TRUE) >= min_cases
  })]
  
  pb <- progress_bar$new(
    format = "  [:bar] :percent | :current/:total clusters ",
  ...
  for (outcome in outcome_cols) {
    ...
    for (alpha in alpha_values) {
      ...
      model_list[[paste0("alpha_", alpha)]] <- tryCatch(
        cv.glmnet(X, y, family = "binomial", alpha = alpha,
                  nfolds = nfolds, type.measure = "auc", keep = TRUE),
        error = function(e) {
    ...
    if (all(is.null(model_list))) {
      cat("  No valid models for cluster:", outcome, "\n")
    ...
    coefs <- as.matrix(coef(best_model$glmnet.fit, s = best_lambda))
    
    selected <- rownames(coefs)[which(coefs != 0)]
    selected <- selected[selected != "(Intercept)"]
    
    if (length(selected) == 0) {
      cat("  No predictors selected for cluster:", outcome, "\n")
      skipped_outcomes <- add_row(skipped_outcomes,
                                  outcome = outcome,
  ...
  if (refit_glm && nrow(elasticnet_results) > 0) {
    ...
    for (outcome in names(results_list)) {
      ...
      glm_formula <- as.formula(paste(outcome, "~", paste(selected_predictors, collapse = "+")))
      ...
      if (is.null(glm_fit) && firth) {
        cat("  Trying Firth for", outcome, "\n")
        glm_fit <- tryCatch(
